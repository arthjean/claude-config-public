...
Run via Bash:
```bash
bunx ctx7@latest docs {library_id} "{focused_query}" [--json]
```

Where `{library_id}` is the ID from step 1 (format: `/org/project` or `/org/project/version`).

**The `docs` command takes NO tuning flags.** The only options are `--json` (machine-readable output) and `--help`. There is no `--mode`, `--limit`, `--topic`, or `--page` - those were removed from the CLI. Passing them produces `error: unknown option` AND a misleading exit code 0 (see error handling below). Relevance, depth, and topic focus are driven entirely by the **free-text query string**, so encode intent there.

**Encode intent into the query (replaces the old flags):**

| Intent | How to phrase the query |
|--------|-------------------------|
| Single function lookup | Name the exact symbol + its module path, e.g. `"ServiceExt::serve_stdio transport setup"` |
| How-to / pattern | State the task + key types, e.g. `"build a stdio MCP server exposing tools with #[tool] handlers"` |
| Migration / changelog | Add the word `migration`/`changelog` + both versions, e.g. `"migration from 0.1 to 1.5 breaking changes"` |
| Broad topic overview | Name the subsystem, e.g. `"transport layer architecture overview"` |

**Query formulation rules:**
- Good: `bunx ctx7@latest docs /tokio-rs/axum "JWT authentication middleware setup with FromRequestParts"`
- Bad: `bunx ctx7@latest docs /tokio-rs/axum "auth"`
- Include the library name, version, and specific topic/symbol in the query, it is your only relevance lever.
...
- Use `--json` when you need to parse results programmatically; otherwise omit it for readable output.
...
1. Do NOT retry with identical parameters, the query string is the only lever, so it MUST change.
2. First try: reformulate with different terminology (e.g., "routing" → "router layer").
3. Second try: broaden or narrow the query, add the exact symbol/module path to narrow, or drop a qualifier to broaden; try the other ID candidate from step 1 (e.g. `/websites/rs_rmcp` vs the versioned `/websites/rs_rmcp_1_5_0_rmcp`) if coverage differs.
...
**CRITICAL - exit code is unreliable.** The ctx7 CLI prints `error: ...` to its output but still exits 0. NEVER infer success from the exit code alone. After every call, inspect the OUTPUT text:
- If it contains `error: unknown option` → you passed a flag the CLI no longer supports. Drop ALL flags except `--json` and re-run. Do NOT count this as a content failure, a coverage gap, or a quota issue.
- If it contains `error:` of any other kind → read the message literally before classifying.
- If it is empty or has no doc snippets → treat as a coverage gap, NOT as quota. Empty ≠ quota.

**Quota exhausted - only when the output literally says so** (`Monthly quota reached`, `quota exceeded`, `429`, `rate limit`):
- First run `bunx ctx7@latest whoami`. If it prints `Logged in`, you are authenticated, `login` will NOT help; the monthly request cap is hit (resets on cycle, or needs a higher plan). Report this precisely; do not tell the user to log in when they already are.
- Only if `whoami` shows NOT logged in, suggest `bunx ctx7@latest login`.
- Then fall back to local source, and escalate to agent-websearch.
- Do NOT label a failure "quota" unless the output text actually contains a quota/rate-limit message. An empty result, an unknown-option error, or a resolution miss is NOT a quota problem.

**Command not found or bunx/npm error:**
- The CLI runs via `bunx ctx7@latest`, no global install needed.
- If bunx itself fails, report the error and escalate to agent-websearch.
...
2. Query docs for the old version via `ctx7 docs /org/project/old-version "{topic} migration changelog breaking changes"`
3. Query docs for the new version via `ctx7 docs /org/project/new-version "{topic} migration changelog breaking changes"`
4. Highlight what changed: new APIs, deprecated APIs, breaking changes, migration steps

**Note:** This mode consumes the full 3-call budget with zero retry capacity. Put `migration`/`changelog` and both version numbers directly in the query string to maximize relevance on the first attempt.
...
3. Bake each sub-topic's keyword directly into its docs query string (the CLI has no `--topic` filter)
