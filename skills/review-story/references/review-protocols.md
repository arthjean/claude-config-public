...
**Assign confidence badges** (these map to the 0-100 scale in SKILL.md Phase 4.5c):
- **HIGH (>= 80)** - Static analysis error OR exact Grep match at cited location. Enters auto-remediation.
- **MEDIUM (50-79)** - Pattern confirmed by reading surrounding context (±10 lines). Reported as an observation, not auto-fixed.
- **LOW (< 50)** - Inferred from research, plausible but ambiguous. Dropped.
...
Only HIGH-confidence (>= 80) CONFIRMED/PARTIAL findings enter Phase 5 auto-remediation. MEDIUM-confidence findings are reported as observations; LOW are dropped.
