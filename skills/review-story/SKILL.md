...
model: opus
name: review-story
description: "End-to-end review and correction workflow for an ALREADY-IMPLEMENTED user story (or full PRD whose stories have been coded). Reviews the CODE against its PRD, not the PRD document. Orchestrates 7 phases: intake, meta-code-style research, static analysis, parallel AI code review + security audit (3-layer: SAST/Secrets/SCA), finding validation, risk-tiered remediation, and executive summary report. Signal-over-noise design: pre-filters generated files, scopes to diff+1 hop, targets 2-4 high-value findings per file. Do NOT use for greenfield scaffolding, code generation, or for reviewing an unimplemented PRD document (this reviews CODE, not the spec). Does not commit or push. Invoke with /review-story [prd-path] [story-id?]."
...
Identify which files implement the stories. Resolve the review base once, then list changed files (reuse this BASE in 1f and 2.5d):

```bash
# Resolve base: origin's default branch, else main, else master.
BASE=$(git symbolic-ref -q --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@'); BASE=${BASE:-main}; git rev-parse -q --verify "$BASE" >/dev/null 2>&1 || BASE=master
if git rev-parse -q --verify "$BASE" >/dev/null 2>&1; then
  git diff --numstat "$BASE...HEAD"                       # added/removed/path vs base merge-base
else
  git diff --numstat HEAD; git diff --numstat --cached    # no base branch: working tree + staged
fi
```

`--numstat` yields `added removed path`, feeding the line counts in 1g. If even the working-tree fallback is empty, ask the user which range to review.
...
```bash
# Reuse BASE from 1c; shell state does not persist across calls, so re-resolve:
BASE=$(git symbolic-ref -q --short refs/remotes/origin/HEAD 2>/dev/null | sed 's@^origin/@@'); BASE=${BASE:-main}; git rev-parse -q --verify "$BASE" >/dev/null 2>&1 || BASE=master
git log --oneline "${BASE}..HEAD"      # commits on this branch (two-dot)
git log --format="%B" "${BASE}..HEAD"  # full messages with context
```
...
**Threshold (default 80):** Only findings scoring >= 80 (HIGH confidence) enter auto-remediation. Findings scoring 50-79 (MEDIUM) are reported as observations, not auto-fixed: the pipeline is autonomous and never pauses to ask, so uncertain findings are surfaced rather than queued. Findings below 50 are dropped.
...
**Key rules** (autonomous pipeline: log-and-proceed everywhere, never pause for confirmation):
- **Scope guard** (5a): >7 fix ops OR >20 files OR >800 LOC delta → log the scope warning, then proceed ([Scope Guard](@~/.claude/skills/_shared/scope-guard.md) thresholds, non-interactive variant)
- **Triage** (5b): CRITICAL/HIGH/MUST_FIX → fix immediately. SHOULD_FIX → report only (never enters remediation loop)
- **Risk tiers** (5c): LOW (auto-fix), MEDIUM (fix + show diff), HIGH (apply with extra caution, log rationale)
- **Max 3 iterations** (5d): fix → re-test → fix regressions. After 3: stop and report remaining issues
- **Scope creep guard** (5e): fix touches files outside scope → log a warning, proceed only if directly required
- **Exit verification** (5f): fresh-context `agent-explore` verifies MUST_FIX/CRITICAL/HIGH fixes resolved
- **Escalation** (5g): remaining issues → report to the user with context (no blocking prompt)
...
**Autonomy model:** This pipeline runs only on explicit invocation and is autonomous by design. Every point a non-autonomous reviewer would pause for confirmation (HIGH-risk fixes, scope-guard breaches, scope creep, MEDIUM-confidence findings) is converted to LOG-AND-PROCEED: it logs a rationale and continues, never blocking on a prompt. The one structural ASK FIRST that survives is "push to remote", which cannot occur because the pipeline never commits or pushes.

- **ALWAYS:** Research before review. Static analysis before AI review. Validate findings (Phase 4.5) before remediation. Compress context at phase boundaries. Keep agent outputs tight: roughly 1,000 tokens (websearch/explore), 800 (docs), 1,500 (review/security), prioritizing top findings. Pre-filter lock/generated/vendor files. Update status JSON. Decide autonomously, never ask the user.
